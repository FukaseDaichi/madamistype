...
- `v3` は名前バイト列 + 4 軸トレンド状態を 3 byte に pack した compact 形式
- `v1` / `v2` は JSON を UTF-8 化して Base64URL 化する

### 9.4 共有ページでの復元

- `v1`: ユーザー名のみ復元
- `v2`: 回答内容から現行ロジックで再計算して 4 軸サマリを得る
...
- `v2` は現行の質問マスタ / 判定ロジックで再計算するため、将来ロジック変更時に旧 URL が影響を受ける余地がある
