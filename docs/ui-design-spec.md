...
本書は [specification.md](./specification.md) の「8. 主要画面詳細」「9. 非機能要件」「11. 文言・表現方針」をUI観点で具体化する補助仕様書である。要件の主文書は [specification.md](./specification.md) とし、本書はそれを単独で上書きしない。
...
- ユーザー名入力を開始CTAより前に置き、「最初に入力する項目」であることを明確にする
- ユーザー名入力は `input type="text"` を使い、`maxLength=10` を設定する
- ラベルは「ユーザー名」よりも「お名前」など自然な日本語を優先する
- 空欄または10文字超過では開始CTAを活性化しない
...
- MVPでは1ページ8問を原則とする
- ページ上部で「質問 9〜16 / 32」のように現在の範囲が分かるようにする
- 各設問は `<fieldset>` + `<legend>` を使って縦に積み、設問文と選択肢のまとまりを明確にする
- 各設問文はスクロール中でも見失いにくいよう、本文より一段強く見せる
- 選択肢は「比較」がしやすいよう、サイズと行頭位置を揃える
- 選択肢の最小タップ高は **52px**、選択肢間のギャップは **12px** とする
- 前へは常に見えるが、主役にしない
- 長い選択肢でも2〜3行で収まりやすい幅と行間を取る
- 補足文は未読でも回答可能な位置づけにする
- 進捗表示は `aria-live="polite"` で「ページ 2 / 4、質問 9〜16 / 32」のようにスクリーンリーダーへ通知する
- ページ遷移時はページ見出しへ、未回答エラー時は最初の未回答設問の `<legend>` へフォーカスを移動する
- `localStorage` に保存した途中状態が復元された場合は、ユーザーが再開できることを短い文言で伝える
...
- ユーザー名はタイプ名称の補助情報として扱い、「○○さんの診断結果」のように自然な日本語で見せる
...
- **各タイプのキービジュアル配色は `visualProfile.colorPalette` をベースに決める**（イラスト、結果カード、OGP画像に一貫して適用する）
...
| ページ間のスライド遷移 | 200-250ms | ease-in-out |
...
- 現在のページ: 左にスライドアウト + フェードアウト（200ms, ease-in）
- 次のページ: 右からスライドイン + フェードイン（250ms, ease-out）
- 戻る操作時は方向を逆転する
...
- OGP画像に含める要素: タイプ名称、キャラクターイラスト、タイプ別キービジュアル配色（`visualProfile.colorPalette` をベースに決定）、サービスロゴ
...
- シェアURLは `/types/[typeCode]/[KEY]` を基本とする
- `KEY` はユーザー名を含む可逆な共有用文字列とし、将来は回答内容も含められる前提で設計する
- `KEY` 付きURLは共有用の結果ページとし、一般公開用の固定詳細ページとは役割を分けて扱う
...
- ページ遷移時にフォーカスを新しいページの見出しへ移動し、未回答エラー時は該当設問の `<legend>` に移動する
- 結果表示時にフォーカスをタイプ名称の見出しに移動する
- モーダル/オーバーレイ表示時はフォーカスをトラップし、閉じた後にトリガー要素にフォーカスを戻す

### 12.3 スクリーンリーダー対応

- 進捗状況の更新は `aria-live="polite"` で通知する（例: 「ページ 2 / 4、質問 9〜16 / 32」）
